...
"void main()\n"
"{\n"
"   outColor = mix(texture(texPuppy, Texcoord), texture(tex_preview, Texcoord), 1.0);//* vec4(color, 1.0);\n"
"}\n";
...
int main(void)
{
    ...
    if (count <= 0)
    {
        ...
        // glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, 1056, 704, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
        ...
        while (1)//!glfwWindowShouldClose(window))
        {
            ...
            // glTexSubImage2D(GL_TEXTURE_2D, textures[1], 0, 0, _width, _height, GL_RGB, GL_UNSIGNED_BYTE, buffer);
            // //glUniform1i(glGetUniformLocation(program, "tex_preview"), 1);
