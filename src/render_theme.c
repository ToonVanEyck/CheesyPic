...
#define SVGFILE "../default.theme.svg"
...
int render_theme_layer(theme_t *theme, xmlNode *layer, overlay_t *overlay)
{
    ...
    cairo_set_source_rgb(theme_ctx, 1, 1, 1);
    ...
    // cairo_surface_write_to_png (theme_surface, filename);
    overlay->height = cairo_image_surface_get_height(theme_surface);
    overlay->width = cairo_image_surface_get_width(theme_surface);
    size_t img_size = (sizeof(uint32_t) * overlay->width * overlay->height);
    LOG("w:%d x h:%d  tot:%lu\n",overlay->width,overlay->height,img_size);
    overlay->data = malloc(img_size);
    if(!overlay->data){
        LOG("Failed to allocate data for theme image!\n");
        return 1;
    }
    memcpy(overlay->data,cairo_image_surface_get_data(theme_surface),img_size);
...
int main()
{
    theme_t theme;
    memset(&theme,0,sizeof(theme_t));
    load_theme_from_file(&theme, SVGFILE);
    free_theme(&theme);
    return 0;
}